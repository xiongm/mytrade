...
def test_update_global_index_highlights_latest_run(tmp_path):
    ...
    # Create two bundles
    for i, fingerprint in enumerate(["abc123", "def456"]):
        bundle_dir = strategy_dir / "bundles" / fingerprint
        bundle_dir.mkdir(parents=True)
        (bundle_dir / "summary.json").write_text(json.dumps({
            "base": {"total_return": 0.1 * (i + 1), "win_rate": 0.7, "max_drawdown": -0.02}
        }))
        
        # Create history record
        history_dir = strategy_dir / "history"
        history_dir.mkdir(parents=True, exist_ok=True)
        timestamp = f"2026-04-18T14-10-0{i}"
        (history_dir / f"{timestamp}.json").write_text(json.dumps({
            "timestamp": timestamp,
            "strategy": "mean_reversion_v1",
            "market": "us",
            "instrument_type": "etf",
    ...
    # Create mock latest pointer for the last one
    latest_dir = strategy_dir / "latest"
    latest_dir.mkdir(parents=True)
    (latest_dir / "latest.json").write_text(json.dumps({
        "strategy": "mean_reversion_v1",
        "timestamp": "2026-04-18T14-10-01",
    ...
    # Check if both are in Recent Activity
    assert "10.00%" in content
    assert "20.00%" in content
    
    # Check if the latest one (20.00%) has the highlight color
    assert "background: #fffde7;" in content
